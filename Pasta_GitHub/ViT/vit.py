...
def read_rgb(path):
    ...
    img = tf.image.resize(img, (IMAGE_HEIGHT, IMAGE_WIDTH), method='bilinear')
    return tf.cast(img, tf.float32) / 255.0  # normaliza para [0,1]

def read_mask(path):
    m = tf.io.read_file(path)
    m = tf.image.decode_png(m, channels=1)
    m = tf.image.resize(m, (IMAGE_HEIGHT, IMAGE_WIDTH), method='nearest')
    return tf.cast(m, tf.int32)

def load_pair(pi, pm):
    return read_rgb(pi), read_mask(pm)
...
def augment_pair(x, y):
    ...
    nh = tf.cast(IMAGE_HEIGHT * scale, tf.int32)
    nw = tf.cast(IMAGE_WIDTH  * scale, tf.int32)
    x = tf.image.resize(x, (nh, nw), method='bilinear')
    y = tf.image.resize(y, (nh, nw), method='nearest')
    def _crop():
        xx = tf.image.random_crop(x, [IMAGE_HEIGHT, IMAGE_WIDTH, 3])
        yy = tf.image.random_crop(y, [IMAGE_HEIGHT, IMAGE_WIDTH, 1])
        return xx, yy
    def _pad():
        xx = tf.image.resize_with_crop_or_pad(x, IMAGE_HEIGHT, IMAGE_WIDTH)
        yy = tf.image.resize_with_crop_or_pad(y, IMAGE_HEIGHT, IMAGE_WIDTH)
        return xx, yy
    return tf.cond(
        tf.logical_and(nh >= IMAGE_HEIGHT, nw >= IMAGE_WIDTH),
        _crop,
        _pad
    )
...
def cutmix(batch_x, batch_y):
    ...
    mask = tf.pad(mask[y1:y2, x1:x2], [[y1, H - y2], [x1, W - x2], [0, 0]])
    inv = 1.0 - mask
...
test_ds = test_ds.map(load_pair, num_parallel_calls=tf.data.AUTOTUNE)
...
def DilatedSpatialPyramidPooling(x):
    ...
    b0 = convolution_block(x, k=1, rate=1)
...
# Deeplabv3+ com ViT de backbone
def DeeplabV3Plus_ViT(num_classes):
    inp = keras.Input((IMAGE_HEIGHT, IMAGE_WIDTH, 3), name="input_image")
    x = inp * 255.0

    vit_backbone_full = vit.vit_b16(
        image_size=(IMAGE_HEIGHT, IMAGE_WIDTH),
        pretrained=True,
        include_top=False,
        pretrained_top=False 
    )
    vit_backbone_full.trainable = False
    vit_output = vit_backbone_full.layers[-2].output
    vit_backbone = keras.Model(inputs=vit_backbone_full.input, outputs=vit_output)

    t = vit_backbone(x)
    t = layers.Lambda(lambda z: z[:,1:,:])(t)

    dim    = int(t.shape[-1])
    grid_h = IMAGE_HEIGHT // PATCH_SIZE
    grid_w = IMAGE_WIDTH  // PATCH_SIZE

    x = layers.Reshape((grid_h, grid_w, dim))(t)
    x = DilatedSpatialPyramidPooling(x)
    x = tf.image.resize(x, (IMAGE_HEIGHT, IMAGE_WIDTH), method='bilinear')
    logits = layers.Conv2D(num_classes, 1, padding='same',
                           kernel_initializer='he_normal',
                           name="logits")(x)

    return keras.Model(inputs=inp, outputs=logits, name="DeepLabV3Plus_ViT")

model = DeeplabV3Plus_ViT(num_classes)
model.summary()
...
if optimizer_choice == 1:
    ...
    optimizer = keras.optimizers.SGD(
        learning_rate=initial_lr, momentum=momentum
    )
...
callbacks = [
    ...
    # EarlyStopping
    EarlyStopping(monitor=es_monitor, mode='max',
