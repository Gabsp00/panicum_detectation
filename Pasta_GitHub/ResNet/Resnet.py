...
from tensorflow.keras.applications.resnet import preprocess_input as resnet_preprocess, ResNet50, ResNet101, ResNet152
...
# Deeplabv3+ com Resnet de backbone
def DeeplabV3Plus_ResNet(num_classes):
    inp = keras.Input((None, None, 3), name="input_image")
    x = (inp + 1.0) * 127.5
    ...
    x = tf.image.resize(x, tf.shape(inp)[1:3], method='bilinear')
    logits = layers.Conv2D(num_classes, 1, padding='same', kernel_initializer='he_normal')(x)
    return keras.Model(inputs=inp, outputs=logits, name="DeepLabV3Plus_ResNet")

model = DeeplabV3Plus_ResNet(num_classes)
model.summary()
